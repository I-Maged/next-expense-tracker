...
### Auth models (generated by `bunx auth generate` — do not hand-write)

`users`, `sessions`, `accounts`, `verifications` — owned by Better-Auth.
...
- `middleware.ts` checks session-cookie presence only (no DB call) for redirect
